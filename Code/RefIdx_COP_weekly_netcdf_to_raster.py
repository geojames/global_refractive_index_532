...
# 1 file example
nc_files = ['H:/RefractiveIndex_copy/Copernicus/weeklyRI/dataset-armor-3d-nrt-weekly_20230101_20231228.nc']
...
for ncf in nc_files:
    ...
    for year in ncf_dates.date.dt.year.unique():
        ...
        #   it does not exist
        out_path = "%s/weekly_rasters"%(os.path.split(ncf)[0])
        ...
        for i,date in enumerate(ncf_dates.date[ncf_dates.date.dt.year == year]):
            ...
            dst = rio.open('%s/armor3d_nw532_%s_weekly_%s.tif'%(out_path,date_string,prefix),
                          'w', driver='GTiff', height=lat.shape[0], 
                          width=long.shape[1], count=1, dtype=np.dtype('float32'), 
            ...
            dst.write_band(1, ri)
            dst.set_band_description(1,date_string)
